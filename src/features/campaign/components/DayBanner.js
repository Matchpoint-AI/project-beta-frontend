...
var DayBanner = function (_a) {
  ...
  var index = _a.index,
    ...
    content = _a.content,
    generatedContentId = _a.generatedContentId,
    handleApprovalUpdate = _a.handleApprovalUpdate,
    setOpen = _a.setOpen,
  ...
  var _d = useState(
      content.posts.map(function (post) {
        return post.text_versions
          ? post.text_versions.findIndex(function (text) {
    ...
    selectedImages = _d[0],
    setSelectedImages = _d[1];
  var _e = useState(1),
    maxVisiblePosts = _e[0],
    setMaxVisiblePosts = _e[1];
  ...
  useEffect(
    function () {
      var _a;
      if (
        scrollContainerRef === null || scrollContainerRef === void 0
          ? void 0
          : scrollContainerRef.current
      ) {
        var postWidth = scrollContainerRef.current.scrollWidth / content.posts.length;
        (_a = scrollContainerRef.current) === null || _a === void 0
          ? void 0
          : _a.scrollTo({
  ...
  var handleApprove = function (week, day, content) {
    return __awaiter(void 0, void 0, void 0, function () {
      var endpointUrl, userTimezone, validTimezone, payload, response, errorData, error_1;
  ...
  return _jsxs('div', {
    // key={index}
    className:
      'bg-[#FFFFFF] border border-[#E5E7EB] flex flex-col gap-3 shadow-md p-6 w-full rounded-md mt-0 ',
    children: [
      _jsxs('div', {
        className: 'flex flex-row justify-between items-center',
        children: [
          _jsxs('p', {
            className: 'text-[#1F2937] font-semibold text-lg',
            children: [
              'Day ',
              index + 1,
              ' |',
              ' ',
              _jsx('span', {
                className: 'text-[#1F2937] font-medium text-lg',
                children: formatDate(startDate, index),
              }),
            ],
          }),
          content.approved === true
            ? _jsx('button', {
                ...
                disabled: true,
                className:
                  ' text-[#356751] font-semibold text-base text-center border-[1px] border-[#046C4E] bg-[#DEF7EC] px-2 py-2 rounded-md',
                children: 'Approved',
              })
            : _jsx('button', {
                onClick: function () {
                  return handleApprove(currentPage, index, content);
                },
                // disabled={true}
                className:
                  'text-[#8E4B10] font-semibold text-base text-center border-[1px]  border-[#8E4B10] bg-[#FDF6B2] hover:bg-[#fdf49f]  md:px-2 md:py-2 rounded-md',
                children: fullLoading
                  ? _jsx(CircularProgress, { sx: { color: '#6C2BD9' }, size: 25, thickness: 5 })
                  : 'Approve Full Day',
              }),
        ],
      }),
        ...
        children: [
          showNavigation &&
            _jsx('button', {
              ...
              className:
                'left-0 top-1/2 -translate-y-1/2 z-10 bg-white absolute rounded-full shadow-md p-2 '.concat(
                  canScrollLeft
                    ? 'text-[#6C2BD9] hover:bg-gray-100'
                    : 'text-gray-300 cursor-not-allowed'
                ),
              style: { transform: 'translate(-50%, -50%)' },
              children: _jsx(FaArrowLeft, { size: '20px' }),
            }),
          _jsx('div', {
            ref: scrollContainerRef,
            className:
              'overflow-x-auto hide-scrollbar mx-auto w-[310px] md:w-[580px] lg:w-[860px] xl:w-[1100px] 2xl:w-[1710px]',
            style: { scrollbarWidth: 'none', msOverflowStyle: 'none' },
            children: _jsx('div', {
              className: 'w-full h-full flex flex-row gap-0 md:gap-4 lg:gap-2 xl:gap-4 ',
              children: content.posts.map(function (post, postIndex) {
                var postTime = postingTimes[postIndex] || 'Unscheduled';
                return _jsx(
                  'div',
                  {
                    className:
                      'w-full md:w-[calc(33.333%-1rem)] min-w-[280px] mx-auto flex-shrink-0',
                    children: _jsx(
                      SocialMediaPost,
                      {
                        day: index,
                        ...
                        onApprovalUpdate: function (postIndexx, isApproved) {
                          return handleApprovalUpdate(
                            currentPage - 1,
                            index,
                            postIndex,
                            isApproved
          ...
          showNavigation &&
            _jsx('button', {
              ...
              className:
                'absolute right-0 top-1/2 -translate-y-1/2 z-10 bg-white rounded-full shadow-md p-2 '.concat(
                  canScrollRight
                    ? 'text-[#6C2BD9] hover:bg-gray-100'
                    : 'text-gray-300 cursor-not-allowed'
                ),
              style: { transform: 'translate(50%, -50%)' },
              children: _jsx(FaArrowRight, { size: '20px' }),
            }),
        ],
      }),
    ],
