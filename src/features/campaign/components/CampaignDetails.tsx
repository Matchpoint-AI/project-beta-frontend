    ...
    <>
      {error && (
        <div className="p-5 mb-[14px] bg-[#fff5f5] border border-[#F05252] rounded-md">
          <p className="text-[#F05252] font-medium">Error loading campaign details</p>
        </div>
      )}
      {loading && (
        <div className="p-5 mb-[14px] flex items-center justify-center flex-col gap-1">
          <CircularProgress sx={{ color: '#42389D' }} size={25} thickness={5} />
          <p className="text-[13px] capitalize text-[#6B7280]">loading campaign details</p>
        </div>
      )}
      {specs && (
        <>
