...
var DateCell = function (props) {
    ...
    return (_jsx("div", { className: "font-medium text-base border-1 w-fit h-fit border-gray-200 float-end", children: parseData() }));
};
var CalendarCampaign = function (_a) {
    var campaign = _a.campaign;
    var localizer = useMemo(function () { return momentLocalizer(moment); }, []);
    ...
    useEffect(function () {
        ...
        var createEvents = function (weekDataArray) {
            ...
            weekDataArray.forEach(function (weekData) {
                var eventsForWeek = Array.from({ length: 7 }, function (_, index) {
                    ...
                    if (dayData.approved)
                        status = 'approved';
                    else if (!dayData.approved && dayData.posts_to_generate === 0)
                        status = 'review';
                    else if (!dayData.approved && dayData.posts_to_generate > 0)
                        status = 'generate';
                    // Get all keys that start with "post_"
                    var postKeys = Object.keys(dayData).filter(function (key) { return key.startsWith('post_'); });
                    ...
                    var postsForDay = postKeys.map(function (key, i) { return (__assign(__assign({}, dayData[key]), { timing: timeSlots[i] || timeSlots[timeSlots.length - 1] })); });
                    return {
        ...
        var fetchContentGenerated = function () { return __awaiter(void 0, void 0, void 0, function () {
            var response, data, eventsArr, _error_1;
    ...
    return (_jsx("div", { className: "h-screen mt-5 mb-5 py-0 text-gray-900", children: _jsx(Calendar, { className: "border-none", ref: calendarRef, culture: "en-US", localizer: localizer, events: events, components: components, views: ['month'], defaultView: "month" }) }));
};
