  ...
  return (
    <div className="w-full h-16 flex flex-row justify-center items-center mb-6">
      <div className="w-fit h-10 flex flex-row gap-4 items-center">
        <IoIosArrowBack className="w-5 h-5 hover:cursor-pointer" onClick={handleBackward} />

        {/* Month Dropdown */}
        <select value={selectedMonth} onChange={handleMonthChange} className="p-1 border rounded">
          {months.map((month) => (
        ...
        <IoIosArrowForward className="w-5 h-5 hover:cursor-pointer" onClick={handleForward} />
      </div>
    </div>
  );
};
