...
var ScoreCircle = styled(Box)(function (_a) {
  ...
  return {
    ...
    background: 'conic-gradient(\n    '
      .concat(getScoreColor(score), ' ')
      .concat(score * 3.6, 'deg, \n    rgba(255, 255, 255, 0.2) ')
      .concat(score * 3.6, 'deg\n  )'),
    position: 'relative',
    '&::before': {
      content: '""',
...
function getScoreIcon(score) {
  if (score >= 80) return _jsx(CheckCircle, { sx: { fontSize: 16 } });
  if (score >= 60) return _jsx(Warning, { sx: { fontSize: 16 } });
  return _jsx(ErrorIcon, { sx: { fontSize: 16 } });
}
export var QualityScoreIndicator = function (_a) {
  var content = _a.content,
    _b = _a.contentType,
    contentType = _b === void 0 ? 'caption' : _b,
    brandContext = _a.brandContext,
    onScoreChange = _a.onScoreChange,
    _c = _a.minHeight,
    minHeight = _c === void 0 ? 120 : _c,
    _d = _a.showDetails,
    showDetails = _d === void 0 ? true : _d,
    _e = _a.position,
    position = _e === void 0 ? 'inline' : _e;
  var _f = useState(0),
    overallScore = _f[0],
    setOverallScore = _f[1];
  var _g = useState([]),
    metrics = _g[0],
    setMetrics = _g[1];
  var _h = useState(false),
    isCalculating = _h[0],
    setIsCalculating = _h[1];
  var _j = useState(false),
    expanded = _j[0],
    setExpanded = _j[1];
  var _k = useState(0),
    previousScore = _k[0],
    setPreviousScore = _k[1];
  // Analyze content quality
  var analyzeQuality = useCallback(
    function (text) {
      var results = [];
      // Length Check
      ...
      var keywordScore = calculateKeywordScore(
        text,
        (brandContext === null || brandContext === void 0 ? void 0 : brandContext.keywords) || []
      );
      results.push({
        name: 'Keywords',
        score: keywordScore,
      ...
      // Tone Alignment
      var toneScore = calculateToneScore(
        text,
        (brandContext === null || brandContext === void 0 ? void 0 : brandContext.tone) || 'neutral'
      );
      results.push({
        name: 'Tone',
        score: toneScore,
        ...
        suggestions: getToneSuggestions(
          text,
          (brandContext === null || brandContext === void 0 ? void 0 : brandContext.tone) ||
            'neutral'
        ),
      });
      return results;
  ...
  var debouncedCalculate = useMemo(
    function () {
      return debounce(function (text) {
        ...
        setTimeout(function () {
          ...
          var newScore = Math.round(
            newMetrics.reduce(function (acc, metric) {
  ...
  return _jsx(
    FloatingWrapper,
    __assign({}, floatingProps, {
      children: _jsx(QualityCard, {
        sx: { minHeight: minHeight, transition: 'all 0.3s ease' },
        children: _jsxs(CardContent, {
          children: [
                          ...
                          children: [
                            ...
                            scoreTrend !== 'stable' &&
                              _jsx(Box, {
                                ...
                                children:
                                  scoreTrend === 'up'
                                    ? _jsx(TrendingUp, { sx: { fontSize: 16, color: '#4caf50' } })
                                    : _jsx(TrendingDown, {
            ...
            _jsx(Collapse, {
              in: expanded || !showDetails,
              children: _jsx(Box, {
                sx: { mt: 2 },
                children: metrics.map(function (metric, _index) {
                  return _jsxs(
                    Box,
                    {
                      sx: { mb: 1.5 },
                            ...
                            _jsxs(Box, {
                              ...
                              children: [
                                ...
                                _jsx(Tooltip, {
                                  title: _jsxs(Box, {
                                    children: [
                                      ...
                                      metric.suggestions &&
                                        ...
                                        _jsx(Box, {
                                          mt: 1,
                                          children: metric.suggestions.map(
                                            function (suggestion, _i) {
                                              return _jsxs(
                                                Typography,
                                                {
                                                  variant: 'caption',
                                  ...
                                  children: _jsx(Info, { sx: { fontSize: 14, opacity: 0.7 } }),
                                }),
                              ],
                            }),
                          ],
...
function calculateKeywordScore(text, keywords) {
  ...
  var foundKeywords = keywords.filter(function (kw) {
...
function calculateToneScore(text, targetTone) {
  ...
  var lowerText = text.toLowerCase();
  var matches = indicators.filter(function (word) {
    return lowerText.includes(word);
  });
...
function getKeywordSuggestions(text, keywords) {
  ...
  var lowerText = text.toLowerCase();
  var missingKeywords = keywords.filter(function (kw) {
    return !lowerText.includes(kw.toLowerCase());
  });
