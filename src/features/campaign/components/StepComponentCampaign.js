...
var StepCampaignComponent = function (_a) {
    ...
    isActive = _a.isActive, currentStep = _a.currentStep, globalStep = _a.globalStep;
    ...
    var handleSave = function () { return __awaiter(void 0, void 0, void 0, function () {
        var info, campaignId, apiEndpoint, features, productLinkToUse, selectedProduct, products, scraped, _err_1, payload, date, year, month, day, hours, minutes, seconds, formattedDateTime, status;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    ...
                    products = [];
                    if (Array.isArray(businessInfo.products) &&
                        businessInfo.products.length > 0 &&
                        businessInfo.products.every(function (item) { return typeof item === 'object' && item !== null; })) {
                        products = businessInfo.products.filter(isProduct);
                    }
                    if ((!productLinkToUse || productLinkToUse === '') && products && info.product) {
                        selectedProduct = products.find(function (p) { return p.name === info.product; });
                        if (selectedProduct) {
                ...
                case 2:
                    ...
                    return [3 /*break*/, 5];
                case 3:
                    if (!(info.product || (selectedProduct && selectedProduct.description))) return [3 /*break*/, 5];
                    return [4 /*yield*/, scrapeProduct(undefined, info.product || (selectedProduct === null || selectedProduct === void 0 ? void 0 : selectedProduct.name), info.product_description || (selectedProduct === null || selectedProduct === void 0 ? void 0 : selectedProduct.description))];
                case 4:
                    scraped = _a.sent();
                    _a.label = 5;
                case 5:
                    if (scraped) {
                        ...
                        setCampaignInfo(function (prev) { return (__assign(__assign({}, prev), { product_features: features })); });
                    }
                    return [3 /*break*/, 7];
                case 6:
                    _err_1 = _a.sent();
    ...
    return (_jsxs("div", { className: "flex flex-col justify-center", children: [_jsxs("div", { className: "flex items-center gap-2", children: [isBeforeCurrentStep ? (_jsx("div", { className: "w-4 h-4 ".concat(isActive ? 'bg-[#5145CD]' : 'bg-[#6B7280]', " rounded-full") })) : (_jsx("div", { className: "w-4 h-4 bg-[#5145CD] rounded-full items-center flex justify-center", children: _jsx("img", { src: "/thumbs_up.svg", alt: "step_inactive", width: 10, height: 10 }) })), _jsx("p", { className: "text-lg font-semibold ".concat(isActive
                            ? 'text-[#5145CD]'
                            : "".concat(!isBeforeCurrentStep ? 'text-[#5145CD]' : 'text-[#6B7280]')), children: name })] }), globalStep !== 1 && campaignInfo.campaign_brief !== true && (_jsx("button", { type: "button", onClick: handleSave, disabled: saving, className: "".concat(currentStep !== 5 ? 'hidden' : '', " h-[41px] w-[142.5px] my-5 p-2 text-sm font-medium border border-[#6B7280] text-[#6B7280] bg-white hover:bg-blue-200 focus:ring-4 focus:outline-none focus:ring-blue-300 rounded-lg text-center"), children: saving ? (_jsx(CircularProgress, { sx: { color: 'gray' }, size: 25, thickness: 5 })) : ('Save For Later') }))] }));
};
