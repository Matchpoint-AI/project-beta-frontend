      ...
      <Dialog
         ...
         <div className="p-7 bg-gradient-to-b from-[#F1FDFF] to-[#F5D9FF] h-full overflow-y-auto">
            <div className="flex items-center justify-between">
               <div>
                  {Array.from({ length: content.length }, (_, i) => i).map(
                     (i) => (
                        <button
                           key={i}
                           ...
                           className={`mr-5 font-medium pb-2 ${
                              selectedWeek == i
                                 ...
                                 : "text-[#6B7280]"
                           } hover:border-b-2 hover:border-[#5145CD] hover:text-[#5145CD] transition-all duration-75 `}>{`Week ${
                           i + 1
                        }`}</button>
                     )
                  )}
            ...
            <div>
               {open &&
                  content[selectedWeek].map((d, i) => {
                     return (
                        <DayContentGalery key={i} num={i + 1} content={d} />
                     );
                  })}
            </div>
            <RawData
