...
export default function BusinessForm({
   ...
   const checkValidity = () => {
      if (
         ...
         return false;
      const values = (businessInfo?.values ?? []).filter(
         (c: Selectable) => c.selected
      );
      const persona = (businessInfo?.persona ?? []).filter(
         (c: Selectable) => c.selected
      );
      const toneAndVoice = (businessInfo?.toneAndVoice ?? []).filter(
         (c: Selectable) => c.selected
      );
      ...
      <>
         <FormsContainer>
            <form id="business_form" onSubmit={handleSubmit}>
               ...
               />
               {businessInfo.industry && businessInfo.vertical && (
                  <IndustryVertical />
               )}
