...
const ServiceForm = ({ handleNext, handleBack, review = false, setService }: ServiceFormProps) => {
    ...
    <>
      <FormsContainer>
        <form id="service_form" onSubmit={handleSubmit}>
          ...
          <div className="my-5">
            <label title="" className="block mb-2 text-base sm:text-xl font-medium text-gray-900">
              If you have it, please provide a link to your product or service
            </label>
            <FormInputBox styles={{ backgroundColor: 'white' }} color={'#6B7280'}>
              <input
                value={productLink}
                type="text"
          ...
          <div className="my-8">
            <label title="" className="block mb-2 text-xl font-medium text-gray-900">
              Service or Product description
            </label>
            <SparklesMessage>
              Matchpoint will try to provide a description through detection or the link above.
            </SparklesMessage>
            {(newProduct || !businessInfo?.products || !product?.description) && (
              <textarea
