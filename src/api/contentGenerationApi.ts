...
export const captionApi = {
  ...
  generateCaptions: async (contentId: string, data: {
    ...
    const requestBody = {
      ...
      caption_length: data.max_length && data.max_length > 1500 ? 'long' : 
                     data.max_length && data.max_length < 500 ? 'short' : 'medium',
    ...
    const response = await fetch(
      `${CONTENT_GEN_URL}/api/v1/content/${contentId}/captions`,
      {
        method: 'POST',
        headers: {
  ...
  regenerateCaption: async (
    ...
    const requestBody = {
      ...
      feedback: data.custom_instruction || 
                (data.style === 'shorter' ? 'Make it shorter' :
                 data.style === 'longer' ? 'Make it longer' :
                 data.style === 'different' ? 'Make it different' : 
  ...
  // Validate caption
  validateCaption: async (data: {
    text: string;
    platform?: 'instagram' | 'facebook' | 'twitter' | 'linkedin';
  }, token: string) => {
    const response = await fetch(
      `${CONTENT_GEN_URL}/api/v1/captions/validate`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
...
export const policyApi = {
  // Create a new policy
  createPolicy: async (campaignId: string, data: {
    campaign_id: string;
    ...
    const response = await fetch(
      `${CONTENT_GEN_URL}/api/v1/campaigns/${campaignId}/policy`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
...
export const plannerApi = {
  ...
  createPlan: async (campaignId: string, data: {
    ...
    const response = await fetch(
      `${CONTENT_GEN_URL}/api/v1/campaigns/${campaignId}/plan`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
...
export const imageApi = {
  ...
  generateImage: async (data: {
    prompt: string;
    model?: 'flux-pro' | 'flux-dev' | 'flux-schnell' | 'stable-diffusion-xl' | 'dall-e-3' | 'midjourney';
    style?: 'photorealistic' | 'artistic' | 'digital-art' | 'illustration' | 'cinematic' | 'product-shot' | 'fashion' | 'food';
    aspect_ratio?: string;
    ...
    const response = await fetch(
      `${CONTENT_GEN_URL}/api/v1/images/generate`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
  ...
  remixImage: async (imageId: string, data: {
    modifications?: string;
    preserve_style?: boolean;
    variation_strength?: number;
  }, token: string) => {
    const response = await fetch(
      `${CONTENT_GEN_URL}/api/v1/images/remix/${imageId}`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
  ...
  getModels: async (token: string) => {
    const response = await fetch(
      `${CONTENT_GEN_URL}/api/v1/images/models`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
