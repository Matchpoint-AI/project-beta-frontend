...
export const displayDuration = (startDate: string, duration: string | number): string => {
  ...
  const formattedYear = startYear === endYear ? startYear : `${startYear} - ${endYear}`;
...
export const getPostingSchedule = (numPosts: number): string => {
  ...
  if (numPosts === 2) return `${possibleTimes[0]}, ${possibleTimes[3]} ${userTimeZone}`;
  if (numPosts === 3)
...
export const getPostingScheduleArray = (numPosts: number): string[] => {
  ...
  if (numPosts === 1) return [`${possibleTimes[1]} ${userTimeZone}`];
  if (numPosts === 2)
    return [`${possibleTimes[0]} ${userTimeZone}`, `${possibleTimes[3]} ${userTimeZone}`];
  if (numPosts === 3)
    return [
