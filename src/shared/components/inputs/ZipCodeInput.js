...
var ZipCodeInput = function (_a) {
    var currentValues = _a.currentValues, setCurrentValues = _a.setCurrentValues;
    var _b = useContext(BrandContext), businessInfo = _b.businessInfo, setBusinessInfo = _b.setBusinessInfo;
    var _c = useState([]), myOptions = _c[0], setMyOptions = _c[1];
    var _d = useState(null), zipCodesMap = _d[0], setzipCodesMap = _d[1];
    var _e = useState(''), zipcode = _e[0], setZipCode = _e[1];
    useEffect(function () {
        var myMap = new Map();
    ...
    useEffect(function () {
        ...
        if ((results === null || results === void 0 ? void 0 : results.length) === 1 && results[0] === '') {
            setCurrentValues([]);
    ...
    return (_jsx(Autocomplete, { autoComplete: false, filterOptions: function (x) { return x; }, options: myOptions, multiple: true, freeSolo: true, className: "w-full", value: currentValues, onChange: function (event, newValue) {
            setCurrentValues(newValue);
        }, inputValue: zipcode, onInputChange: function (event, newInputValue) {
            ...
            return tags.map(function (item, index) {
                ...
                return (_jsx(Chip, { variant: "solid", sx: { margin: 0.5 }, style: {
                        backgroundColor: '#BCF0DA',
                        color: 'black',
                        borderRadius: '6px',
                    }, endDecorator: _jsx(ChipDelete, __assign({ variant: "plain", style: {
                            backgroundColor: '#BCF0DA',
                            color: 'gray',
                        } }, tagProps)), children: item }, index));
            });
