...
export function useV2Api() {
  ...
  const campaign = {
     ...
     */
    get: useCallback(async (campaignId: string) => {
     ...
     * Update campaign
     */
    update: useCallback(async (campaignId: string, data: any) => {
      const token = await getToken();
     ...
     * Delete campaign
     */
    delete: useCallback(async (campaignId: string) => {
      const token = await getToken();
     ...
     */
    generateContent: useCallback(async (campaignId: string, options?: any) => {
      const token = await getToken();
     ...
     */
    getContent: useCallback(async (campaignId: string) => {
      const token = await getToken();
     ...
     * List campaigns
     */
    list: useCallback(async (filters?: any) => {
      const token = await getToken();
  ...
  const brand = {
     ...
     * Get brand details
     */
    get: useCallback(async (brandId: string) => {
      const token = await getToken();
     ...
     * Update brand
     */
    update: useCallback(async (brandId: string, data: any) => {
      const token = await getToken();
     ...
     * Delete brand
     */
    delete: useCallback(async (brandId: string) => {
      const token = await getToken();
     ...
     * Crawl website
     */
    crawlWebsite: useCallback(async (data: any) => {
      const token = await getToken();
     ...
     * Get brand knowledge
     */
    getKnowledge: useCallback(async (brandId: string) => {
      const token = await getToken();
