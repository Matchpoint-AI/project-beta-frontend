...
setup('authenticate', async ({ page }) => {
  ...
  const hasLoginForm = await page.locator('input[type="email"], input[name="email"], [data-testid="email-input"]').count() > 0;
  
  if (!hasLoginForm) {
    console.log('No login form found, looking for login button...');
    // Try to find and click a login/sign-in button
    const loginButton = page.locator('button:has-text("Login"), button:has-text("Sign In"), a:has-text("Login"), a:has-text("Sign In"), button:has-text("Get Started")').first();
  ...
  const emailInput = page.locator('input[type="email"], input[name="email"], input[placeholder*="email" i], [data-testid="email-input"]').first();
  ...
  if (hasEmailInput && hasPasswordInput) {
    ...
    if (await submitButton.count() > 0) {
      ...
      try {
        await page.waitForURL(url => !url.pathname.includes('login') && !url.pathname.includes('auth'), { timeout: 10000 });
        console.log('Successfully navigated after login');
      } catch {
        console.log('URL did not change, looking for dashboard elements...');
        try {
          await page.waitForSelector('[data-testid="dashboard"], .dashboard, #dashboard, main, .app-container', { timeout: 5000 });
    ...
    console.log('No login form found, app might not require authentication or is already authenticated');
  }
