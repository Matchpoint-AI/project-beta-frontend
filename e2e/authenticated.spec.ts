...
test.describe('Authenticated User Tests', () => {
  ...
  test('should access dashboard when authenticated', { tag: '@authenticated' }, async ({ page }) => {
  ...
  test('should have user navigation menu', { tag: '@authenticated' }, async ({ page }) => {
    ...
    if (navElements > 0) {
      ...
      const userMenuElements = await page.locator([
  ...
  test('should be able to interact with main features', { tag: '@authenticated' }, async ({ page }) => {
  ...
  test('should load content sections', { tag: '@authenticated' }, async ({ page }) => {
    ...
    const contentSections = await page.locator([
      'section',
      '[role="main"]',
      '.content',
      '.main-content',
    ...
    const visibleContent = await page.locator('section, main, .content').filter({ hasText: /.+/ }).count();
    console.log('Sections with visible content:', visibleContent);
  });

  test('should not show login form when authenticated', { tag: '@authenticated' }, async ({ page }) => {
